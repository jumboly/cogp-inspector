  ...
  return (
    <ul className="tree">
      <Node sel={{ kind: 'file' }} label={sourceName?.split('/').pop() ?? 'file'} meta={formatBytes(file.size)} defaultOpen>
        ...
        <Node sel={{ kind: 'rowGroups' }} label="Row Groups（データ本体）" meta={formatNumber(file.rowGroups.length)} forceOpen={selection?.kind === 'rowGroup' || selection?.kind === 'column'}>
          {file.rowGroups.map((rg) => (
            <Node key={rg.index} sel={{ kind: 'rowGroup', rg: rg.index }} label={`RG ${rg.index}`} meta={`${formatNumber(rg.numRows)} 行`} color={rgColor(rg.index)} forceOpen={selection?.kind === 'column' && selection.rg === rg.index}>
              {rg.columns.map((c, ci) => (
                <Node key={ci} sel={{ kind: 'column', rg: rg.index, col: ci }} label={c.column.name} meta={formatBytes(c.compressedSize)} />
              ))}
            </Node>
          ))}
        </Node>

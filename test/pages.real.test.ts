...
describe.skipIf(!existsSync(SAMPLE))('公式サンプルのページ構造', () => {
  ...
  it('Index は一度読めばキャッシュされ、隣接するものは合体して読む', async () => {
    ...
    expect(first).toMatchObject({ fetched: 8, cached: 0 })
    expect(first.fetchedRanges).toEqual(chunks.map((c) => c.offsetIndex))
    ...
    expect(again).toEqual({ fetched: 0, cached: 8, fetchedRanges: [] })
    expect(reads).toHaveLength(1)
  })
})
